...
-- isSafe []  = True 
-- isSafe [_] = True
-- isSafe (x:y:xs) = checkTrend (x:y:xs) (compare x y)

-- checkTrend :: [Int] -> Ordering -> Bool
-- checkTrend [] _ = True 
-- checkTrend [_] _ = True
-- checkTrend (x:y:xs) trend
--     | abs (x - y) < 1 || abs (x - y) > 3 = False 
--     | compare x y /= trend = False
--     | otherwise = True && checkTrend (y:xs) trend

isSafe :: [Int] -> Bool
isSafe list = 
    let diff = zipWith (-) list (tail list)
    in isSafe' Nothing diff

valid :: Int -> Bool
valid x = (x /= 0) && (abs x) <= 3

isSafe' :: Maybe Int -> [Int] -> Bool
isSafe' Nothing (x:xs) = isSafe' (Just x) xs
isSafe' (Just p) [] = valid p
isSafe' (Just p) (x:xs) = (p * x >= 0) && (valid p) && (isSafe' (Just x) xs) 

countSafeReport :: [[Int]] -> Int
countSafeReport [] = 0 
countSafeReport (x:xs) = (if (isSafe x) then 1 else 0) + (countSafeReport xs)

-- Part Two
isSafeTolerance :: [Int] -> Bool
isSafeTolerance list
    | isSafe list = True
    | otherwise = 
    let removeOne :: [Int] -> [[Int]]
        -- removeOne list = [take i list ++ drop (i + 1) list | i <- [1.. (length list) - 1]]
        removeOne [] = [[]]
        removeOne (x:xs) = xs : map (x :) (removeOne xs) -- way more elegant version found on reddit
    in any isSafe (removeOne list)

countSafeReportTolerance :: [[Int]] -> Int 
countSafeReportTolerance [] = 0
countSafeReportTolerance (x:xs) = (if (isSafeTolerance x) then 1 else 0) + (countSafeReportTolerance xs)

main :: IO ()
main = do
  putStrLn "hello world"
  let recordTest = [[7, 6, 4, 2, 1], 
                    [1, 2, 7, 8, 9], 
                    [9, 7, 6, 2, 1], 
                    [1, 3, 2, 4, 5], 
                    [8, 6, 4, 4, 1], 
  ...
  print (countSafeReport record)

  print (countSafeReportTolerance record)
